...
class Item {
  constructor(internal) {
    this.pointlist = internal.pointlist, this.pointItem = document.createElement('li'), this.removeKey = document.createElement('span'), this.itemClassName = internal.itemClassName, this.text = internal.text, this.needFunction = internal.eventFunction, this.id = internal.id;
  }
  ...
  _movetItem(event) {
    ...
    const anotherItems = [...this.pointlist.querySelectorAll('.' + this.itemClassName)].filter(item => item.dataset.id != this.pointItem.dataset.id);
